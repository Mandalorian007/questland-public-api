...
import com.questland.handbook.service.model.questgroups.MobIdAndFilter;
...
public class MonsterSlayerService {
    ...
    public MonsterSlayerScore getStageScores() {
        ...
        Map<MobIdAndFilter, Integer> mobIdAndPoints = getMobIdAndPoints();
    ...
    private MonsterSlayerScore scoreStages(List<BattleLocation> battleLocationDetails,
                                           Map<MobIdAndFilter, Integer> mobIdAndPoints) {
    ...
    private StageScore calculateStageScore(Map<MobIdAndFilter, Integer> mobIdAndPoints,
                                           BattleLocation battleLocation, Stage stage) {
        int score = stage.getStageMobs().stream()
                .mapToInt(mob -> {

                    Map.Entry<MobIdAndFilter, Integer> MobIdFilterAndPoints = findMobIdAndFilterByMobId(mob.getId(), mobIdAndPoints);


                    return getPoints(MobIdFilterAndPoints, battleLocation) * mob.getCount();
                }).sum();
    ...
    private Map.Entry<MobIdAndFilter, Integer> findMobIdAndFilterByMobId(int mobId, Map<MobIdAndFilter, Integer> mobIdAndPoints) {
        return mobIdAndPoints.entrySet().stream().filter(entry -> entry.getKey().getMobId() == mobId)
                .findFirst().orElse(null);
    }

    private int getPoints(Map.Entry<MobIdAndFilter, Integer> mobIdAndFilterEntry, BattleLocation battleLocation) {
        if (mobIdAndFilterEntry != null) {
            MobIdAndFilter mobIdAndFilter = mobIdAndFilterEntry.getKey();
            if (mobIdAndFilter.getMobLocationFilter() == 0) {
                return mobIdAndFilterEntry.getValue();
            } else if (mobIdAndFilter.getMobLocationFilter() == battleLocation.getId()) {
                return mobIdAndFilterEntry.getValue();
            }
        }
        return 0;
    ...
    private Map<MobIdAndFilter, Integer> getMobIdAndPoints() {
    ...
    private MobIdAndFilter extractMobIdFromQuest(JsonNode questObject, int questId) {
        MobIdAndFilter mobIdAndFilter = new MobIdAndFilter();
        int count = 0;
        for (JsonNode node : questObject.path("" + questId)) {
            if (count == 4) {
                mobIdAndFilter.setMobLocationFilter(node.asInt());
            }
            if (count == 11) {
                mobIdAndFilter.setMobId(node.asInt());
            }
            count++;
        }
        return mobIdAndFilter;
    }
